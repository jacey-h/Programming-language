...
def cal_data(array):
    ...
    return block_num, rainbow_cnt, x, y
def bfs(x,y,b_group):
    mem = []
    new_group = []
    q = deque()
    ...
    while q:
        ...
        if (sx,sy) in mem:
            continue
        mem.append((sx,sy))
        if array[sx][sy] != 0 and array[sx][sy] != -1 and color == 0 and array[sx][sy] != (m+1):
            color = array[sx][sy]
        new_group.append([(sx,sy),array[sx][sy]])
        ...
        for d in range(4):
            ...
            if 0 <= nx < n and 0 <= ny < n:
                if color == 0:
                    if array[nx][ny] != -1 and array[nx][ny] != (m+1):
                        q.append((nx, ny))
                elif array[nx][ny] == color or array[nx][ny] == 0:
                    q.append((nx,ny))

    b_data = list(cal_data(b_group))
    new_data = list(cal_data(new_group))
    if new_data[2] == n+1:
        return b_group
    compare_data  = [b_data, new_data]
    compare_data.sort()
    # print(compare_data)

    if compare_data[1] != b_data:
        b_group = new_group

    return b_group

def gravity(array,n,m):
    for x in range(n-2,-1,-1):
        for y in range(n-1,-1,-1):
            if array[x][y] != (m+1) and array[x][y] != -1:
                real_x = x
                real_y = y
                while 1:
                    nx = x + dx[1]
...
while 1:
    ...
    for x in range(n):
        for y in range(n):
            # print((x,y))
            if array[x][y] == -1 or array[x][y] == (m+1):
                continue
            b_group = bfs(x,y,b_group)
    # print(b_group)
    if len(b_group) < 2:
        print(result)
        break
    ###### 블록 그룹 완성
    for i in b_group:
        array[i[0][0]][i[0][1]] = m+1
    result += (len(b_group) * len(b_group))
    array = gravity(array,n,m)
